...
async function addDomainFinal() {
    ...
    try {
        ...
        if (!targetServer) {
            console.log('\n❌ Server with IP 35.184.78.205 not found');
            console.log('💡 Available servers are listed above');
            console.log('🔧 You may need to create a server or check the IP address');
            return;
        }
        ...
        if (applications.length === 0) {
            console.log('\n❌ No applications found on the target server');
            console.log('💡 Please create an application first in your Cloudways dashboard');
            return;
        }

        console.log(`\n📱 Found ${applications.length} applications on server`);
        ...
        console.log('\n🚀 Next Steps:');
        console.log('1. Configure DNS settings (point domain to 35.184.78.205)');
