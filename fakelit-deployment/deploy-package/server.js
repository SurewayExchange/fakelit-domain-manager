...
app.get('/counselors', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'counselors.html'));
});

app.get('/counselor/:id', (req, res) => {
  const counselorId = req.params.id;
  const counselorFile = path.join(__dirname, 'public', 'counselor', `${counselorId}.html`);
  
  // Check if the counselor file exists
  if (require('fs').existsSync(counselorFile)) {
    res.sendFile(counselorFile);
  } else {
